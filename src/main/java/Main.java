...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        FileInputStream fin;
        try {
            fin = new FileInputStream(System.getProperty("user.dir")+"/token.txt");
            BufferedReader br = new BufferedReader(new InputStreamReader(fin));
            token = br.readLine();
            br.close();
            System.out.println("Attempting to create RoySipBot using token: " + token);
            RoySipBot bot = new RoySipBot(createClient(token, true));
